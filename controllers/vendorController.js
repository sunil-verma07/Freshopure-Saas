      ...
      if (stock.itemId.toString() === itemId) {
        return {
          ...
          quantity: {
            ...
            packet: quantity?.litre || stock?.quantity?.litre,
          },
        };
      }
      return stock;
  ...
  try {
    ...
    <div style="${generateInlineStyles(styles.container)}">
      <div style="${generateInlineStyles(styles.header)}">
        ...
        <div>
          <h1 style="font-weight:600;font-size:24px;">ORDER</h1>
        </div>
      </div>
      <div style="display:flex;justify-content:space-between">
...
const addItemToStock = catchAsyncError(async (req, res, next) => {
  try {
    ...
    if (stock) {
      ...
      stock.stocks.push({ itemId, quantity: { kg: 0, gram: 0, piece: 0, packet:0, litre:0 } });
      await stock.save();
    } else {
      // Create a new stock entry if the vendor doesn't have any existing stock
      await vendorStock.create({
        vendorId,
        stocks: [{ itemId, quantity: { kg: 0, gram: 0, piece: 0, packet:0, litre:0 } }],
      });
    }
...
const addHotelItem = catchAsyncError(async (req, res, next) => {
  try {
    ...
    for (const itemId of itemIds) {
      ...
      let quantityOfStock = item.totalQuantity.kg + item.totalQuantity.gram/1000 + item.totalQuantity.piece + item.totalQuantity.packet + item.totalQuantity.litre;
...
const changeOrderQuantity = catchAsyncErrors(async (req, res, next) => {
  try {
    ...
    } else if (order.orderedItems[orderedItemIndex].unit === "litre") {
      oldPrice =
        order.orderedItems[orderedItemIndex].price *
        order.orderedItems[orderedItemIndex].quantity.litre; // Multiply total grams with price and store in totalPrice field
    ...
    if (quantity.kg !== 0 || quantity.gram !== 0) {
      ...
      newPrice = order.orderedItems[orderedItemIndex].price * quantity.litre;
...
const updatePrice = catchAsyncError(async (req, res, next) => {
  try {
    ...
    let vendorQuantity =
      ...
      vendorPrice.totalQuantity.litre +
      ...
      if (orderUpdated) {
        order.totalPrice = order.orderedItems.reduce((total, orderedItem) => {
          const itemTotal =
            ...
            (orderedItem.quantity.litre || 0) * orderedItem.litre +
...
const updateVendorItemStock = catchAsyncError(async (req, res, next) => {
  try {
    ...
    itemToBeUpdated.history.forEach((entry) => {
      ...
      totalItemQuantity.litre += entry.quantity.litre || 0;
    ...
    vendorItem.items.forEach((item) => {
      ...
      totalVendorQuantity.litre += item.totalQuantity.litre || 0;
    ...
    const avgPriceOfItem =
      ...
      (itemToBeUpdated?.totalQuantity?.kg +
        ...
        itemToBeUpdated?.totalQuantity?.litre +
...
const updateVendorItemWaste = catchAsyncError(async (req, res, next) => {
  try {
    ...
    itemToBeUpdated.history.forEach((entry) => {
      ...
      totalItemQuantity += entry.quantity.litre || 0;
...
const compiledOrderHotelDetailsPdf = catchAsyncError(async (req, res, next) => {
  ...
  try {
    ...
    const formatQuantity = (quantity) => {
      ...
      if (quantity.litre > 0) parts.push(`${quantity.litre} litre`);
