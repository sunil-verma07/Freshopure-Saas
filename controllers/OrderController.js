  ...
  try {
    ...
    if(cart_doc){
      ...
      for(let item of cart_doc.cartItems){
        let itemSellPrice = await HotelItemPrice.findOne({itemId:item.itemId})

        if(!itemSellPrice){
            return res.status(404).json({ message: 'Item is not linked' });
        }
        orderedItems.push({itemId:item.itemId, price:itemSellPrice.todayCostPrice, quantity:item.quantity})
      }
...
const orderAgain = catchAsyncError(async (req, res, next) => {
  try {
    ...
    if (findOrder) {
      ...
      if (cartPresent) {
        const x = await Cart.updateOne({ UserId: new ObjectId(UserId) }, { $set: { orderedItems: orderedItems } });
      } else {
        const cart = new Cart({
          UserId,
          orderedItems
  ...
  try {
    ...
    if (duration === "sixmonths") {
      ...
      while (currentDate < new Date()) {
        const monthName = new Date(currentDate).toLocaleDateString('en-US', { month: 'short' });
        monthsArray.push({
      ...
      const monthlyTotal = new Map(monthsArray.map(month => [month.month, 0]));
      ...
      orders.forEach(order => {
        ...
        monthlyTotal.set(monthName, monthlyTotal.get(monthName) + order.totalPrice);
      });
...
module.exports = { placeOrder, orderHistory, orderAgain, orderAnalytics, orderPriceAnalytics,itemAnalyticsForHotel,compiledOrderForHotel }