...
const { sendEmailVerification, checkVerification } = require('../utils/sendEmailVerification.js');
...
const register = catchAsyncErrors(async(req, res,next) => {
    ...
    if(!fullName || !email || !password || !phone || !role) {
        return res.status(400).json({success: false, message:'Please enter all feilds properly!'})
    }else{
        ...
        if(user){
            ...
            const newUser = await User.create({email:email,phone:phone,password:hashedPassword,fullName:fullName,roleId:userRole._id})
            ...
            if (error) {
                sendEmailVerification(email, function (error2) {
                    if (error2) {
                        ...
                        res.status(500).json({success:false,error:error+'. Please enter the new code sent to your email.'})
                    }
                });
            } else if (status) {
...
const setProfile = catchAsyncErrors(async (req, res, next) => {
    try {
      const UserId = req.user._id
      const { hotelName, addressLine1, addressLine2, state, city, pinCode, update } = req.body;
      if (update) {
        await User.updateOne({ UserId: new ObjectId(UserId) }, { $set: { fullName: fullName, hotelName: hotelName } })
        ...
        await User.updateOne({ _id: new ObjectId(UserId) }, { $set: { isProfieComplete: true } })
        res.status(200).json({ message: 'User Profile Updated' });
      }
    } catch (error) {
  ...
  const setProfileImage = catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (update && present) {
        await User.updateOne({ UserId: new ObjectId(UserId) }, { $set: { userImage: userImage } })
      } else {
...
module.exports = {login,register,emailVerification,myProfile,logout,setProfile,setProfileImage};